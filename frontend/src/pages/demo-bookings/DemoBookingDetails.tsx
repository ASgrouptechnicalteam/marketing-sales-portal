...
export default function DemoBookingDetails() {
  ...
  const fetchBooking = async () => {
    try {
      const res = await api.get(`/v1/demo-bookings/${id}`);
  ...
  const updateStatus = async (newStatus: string) => {
    ...
    try {
      await api.patch(`/v1/demo-bookings/${id}/status`, { status: newStatus });
      await fetchBooking();
    } catch (err: any) {
  ...
  const handleComplete = async (e: React.FormEvent) => {
    ...
    setActionLoading(true);
    try {
      await api.patch(`/v1/demo-bookings/${id}/status`, {
        status: 'COMPLETED',
        outcome: outcomeData.outcome
      });
      // also update the other fields
      await api.patch(`/v1/demo-bookings/${id}/outcome`, outcomeData);
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center gap-4">
        ...
        <h1 className="text-2xl font-bold text-primary-navy">Site Booking Details</h1>
      </div>
        ...
        <div className="px-4 py-5 sm:px-6 flex justify-between items-center flex-wrap gap-4">
          <div>
            <h3 className="text-lg leading-6 font-medium text-primary-navy">
              {booking.customerName} - {booking.project.name}
            </h3>
            <p className="mt-1 max-w-2xl text-sm text-gray-500">
              Scheduled for {format(new Date(booking.scheduledDate), 'MMMM d, yyyy')} at {booking.scheduledTime}
            </p>
          </div>
          <Badge variant={
        ...
        <div className="border-t border-gray-200 px-4 py-5 sm:px-6">
          <dl className="grid grid-cols-1 gap-x-4 gap-y-8 sm:grid-cols-2">
            ...
            <div className="sm:col-span-1">
              ...
              <dd className="mt-1 text-sm text-gray-900">{booking.associate?.name}</dd>
            </div>
            <div className="sm:col-span-2">
              <dt className="text-sm font-medium text-gray-500">Remarks</dt>
              <dd className="mt-1 text-sm text-gray-900">{booking.remarks || '-'}</dd>
            </div>

            {booking.outcome && (
              <div className="sm:col-span-2 bg-gray-50 p-4 rounded-md">
                <dt className="text-sm font-medium text-gray-700 mb-2">Booking Outcome</dt>
                <dd className="text-sm text-gray-900 whitespace-pre-wrap">{booking.outcome}</dd>
              </div>
            )}
          </dl>
        </div>
        ...
        <div className="fixed inset-0 z-50 overflow-y-auto">
          <div className="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div className="relative z-10 inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6">
              <div>
                <h3 className="text-lg leading-6 font-medium text-gray-900 mb-4">Complete Site Booking</h3>
